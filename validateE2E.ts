...
interface E2EReport {
  ...
  categorizeEndpoint: string;
  searchEndpoint: string;
  suggestReplyEndpoint: string;
  frontendSearch: string;
  frontendCategorization: string;
  frontendSuggestReply: string;
  errors: string[];
}
...
async function validate() {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/emails/123/categorize`, {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/emails/search?q=test`);
    const json = await res.json();
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/emails/123/suggest-reply`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
